...
public class EntityAccessLookupImpl implements EntityAccessLookup {
    ...
    public boolean isSiteOwner(AccountEntity account) {
        return AccountRoles.isOwner(account, roles);
    }

    @Override
    public String roleId(AccountEntity account) {
        return AccountRoles.roleId(account, roles);
    }
    ...
    public BasicEntity parentOf(BasicEntity entity) {
        if (entity instanceof ItemEntity item && item.getRefCategoryId() != null) {
            ...
            return services.getCategoryService().getById(CategoryEntity.class, item.getRefCategoryId());
        }
        if (entity instanceof ResourceEntity resource && resource.refResourceGroupObjectId() != null) {
            ...
            return services.getResourceGroupService()
                    .getById(ResourceGroupEntity.class, resource.refResourceGroupObjectId());
        }
        if (entity instanceof ResourceGroupEntity group && group.refParentGroupObjectId() != null) {
            ...
            return services.getResourceGroupService()
                    .getById(ResourceGroupEntity.class, group.refParentGroupObjectId());
        }
        return null;
    }
