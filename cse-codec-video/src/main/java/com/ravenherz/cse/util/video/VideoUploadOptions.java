...
public final class VideoUploadOptions {
    ...
    public static VideoUploadOptions from(String maxWidth, String maxHeight, String qualityFactor,
            String audioBitrate) {
        int width = even(clamp(parseInt(maxWidth, DEFAULT_MAX_WIDTH), 16, 7680));
        int height = even(clamp(parseInt(maxHeight, DEFAULT_MAX_HEIGHT), 16, 4320));
        float quality = clampQuality(parseFloat(qualityFactor, DEFAULT_QUALITY_FACTOR));
        String audio = audioBitrate(audioBitrate);
