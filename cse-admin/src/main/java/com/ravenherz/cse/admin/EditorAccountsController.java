...
public class EditorAccountsController {
    ...
    public EditorAccountsController(AccountAccessor authSupport, EditorChrome editorChrome, RoleService roles,
    ...
    public String deleteAccount(@RequestParam(value = "id", required = false) String id,
        ...
        try {
            account = (AccountEntity) accountService.getById(AccountEntity.class, EntityId.of(id.trim()));
        } catch (Exception e) {
            response.sendRedirect(request.getContextPath() + "/?error=404");
            return null;
        }

        if (account != null && AccountRoles.isOwner(account, roles)) {
