...
public final class CompanySocialMarkup {
    ...
    public static String toCseUrls(String raw, Predicate<String> include) {
        ...
        String collapsed = raw.replaceAll(":+", Strings.STR_DELIM_COLON);
        StringBuilder inner = new StringBuilder();
        Set<String> seen = new LinkedHashSet<>();
        for (String token : collapsed.split(Strings.STR_DELIM_WS)) {
            if (token == null || token.isBlank()) {
                continue;
            }
            String[] parts = token.split(Strings.STR_DELIM_COLON);
