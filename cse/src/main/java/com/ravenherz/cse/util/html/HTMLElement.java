...
public class HTMLElement {
    ...
    public static class HTMLElementBuilder {
        ...
        public HTMLElementBuilder surroundByContainer(String dynamicElementTitle,
            ...
            element = new HTMLElement(element.getCode(), dynamicElementTitle,
                    visibilityFactorContent, HtmlNames.ELEM_NAME_DIV);
            return this;
        }
    ...
    private HTMLElement(String childElementContent, String dynamicElementTitle,
            String visibilityFactorContent, String elemType) {
        this.attributes = new HashMap<>();
        this.attributes.put(HtmlNames.ATTR_NAME_ID, dynamicElementTitle + Strings.POSTFIX_CONTAINER);
        if (elemType.equals(HtmlNames.ELEM_NAME_DIV)) {
            this.attributes.put(HtmlNames.ATTR_NAME_CLASS, HtmlNames.CSS_CLASS_ALL_WIDE_DIV);
        }
        if ((visibilityFactorContent != null) && (visibilityFactorContent.trim().length() > 0)) {
            this.attributes.put(HtmlNames.ATTR_NAME_STYLE, "display:inline-block");
        } else {
            this.attributes.put(HtmlNames.ATTR_NAME_STYLE, "display:none");
        }
